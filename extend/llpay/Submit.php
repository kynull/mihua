...
class Submit{
    ...
    function buildRequestMysign($para_sort) {
        ...
        file_put_contents("log/pay_log.txt","新的签名:".$prestr."\n", FILE_APPEND);
        ...
        file_put_contents("log/pay_log.txt","签名:".$mysign."\n", FILE_APPEND);
        return $mysign;
    }
    ...
    function buildRequestPara($para_temp) {
        ...
        //return json_encode($para_sort);  // postForm适用
        return $para_sort;                 // apiPost适用
    ...
    function buildRequestParaToString($para_temp) {
        //待请求参数数组
        $request_data = $this->buildRequestPara($para_temp);
        $para = json_encode($request_data);

        //把参数组中所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串，并对字符串做urlencode编码
        $result = createLinkstringUrlencode($para);
     ...
     * @return 提交表单HTML文本
     */
    function buildRequestForm($para_temp, $url, $method, $button_name) {
        //待请求参数数组
        $request_data = $this->buildRequestPara($para_temp);
        $para = json_encode($request_data);
    ...
    /**
     * 建立请求，以模拟远程HTTPS的POST请求方式构造并获取连连支付的处理结果
     * @param $para_temp 请求参数数组
     * @return 连连支付处理结果
     */
    function buildRequestHttps($para_temp,$url) {
        $sResult = '';
        ...
        //远程获取数据
        $result = getHttpsResponsePOST($url, $this->llpay_config['cacert'], $para, trim(strtolower($this->llpay_config['input_charset'])));
     ...
     * @return 连连支付处理结果
     */
    function buildRequestHttp($para_temp,$url) {
        $sResult = '';
        ...
        //远程获取数据
        $result = getHttpResponsePOST($url, $para);
     ...
     * @return 连连支付返回处理结果
     */
    function buildRequestHttpInFile($para_temp, $file_para_name, $file_name,$url) {
        ...
        //远程获取数据
        $sResult = getHttpResponsePOST($url, $this->llpay_config['cacert'], $para, trim(strtolower($this->llpay_config['input_charset'])));
     ...
     * return 时间戳字符串
     */
    function query_timestamp($url) {
        $url = $url . "service=query_timestamp&partner=" . trim(strtolower($this->llpay_config['partner'])) . "&_input_charset=" . trim(strtolower($this->llpay_config['input_charset']));
        $encrypt_key = "";
